...
function platformOf(video) {
  ...
  if (source.includes("twitch") || url.includes("twitch.tv")) {
    return "twitch";
  }
...
function applyTheme(theme) {
  ...
  if (meta) {
    meta.content = next === "light" ? "#eceef2" : "#111214";
  }

  const toggle = $("#themeToggle");

  if (toggle) {
    toggle.textContent = next === "light" ? "ダーク" : "ライト";
  }
...
async function loadStreamers() {
  if (!supabaseClient || !currentUser || previewMode) {
    return;
  }
  ...
  try {
    ...
    console.error("loadStreamers error:", error);
    ...
    >
      ...
      <div class="sched-meta">
        ...
        <div class="sched-time">
          ${esc(fmtClock(scheduledAt(video)))}〜
        </div>
      </div>
    </article>
  `;
...
function renderSchedule(rows, selector, limit = 0) {
  ...
  for (const video of items) {
    ...
    if (!byDay.has(key)) {
      byDay.set(key, {
        label: fmtDay(scheduledAt(video)),
        videos: [],
      });
    }
...
async function removeStreamer(streamerId) {
  if (!supabaseClient || !currentUser || !streamerId) {
    return;
  }
...
async function init() {
  ...
  try {
    ...
    showAuthMessage("ログイン状態を確認できませんでした。");
  }
}
